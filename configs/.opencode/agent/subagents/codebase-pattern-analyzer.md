...
grep -r "functionName\|className\|patternName" app/
grep -r "import.*ComponentName" app/
grep -r "export.*functionName" app/
```

#### **Secondary Search** (Related Patterns)
```bash
# Search for similar concepts
grep -r "create\|add\|new" app/
grep -r "update\|edit\|modify" app/
grep -r "delete\|remove\|destroy" app/
grep -r "get\|fetch\|load" app/
```

#### **Tertiary Search** (Structural Patterns)
```bash
# Search for file organization patterns
find lib/ -name "*.ts" -o -name "*.service.*" -o -name "*.utils.*" -o -name "*.tsx"
